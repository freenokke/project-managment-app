...
const Footer = () => {
  return (
    <div className="bg-blue-700">
      <div className="container flex wrap justify-between items-center p-1 w-[100%] font-sans">
        ...
        <div className="flex flex-col items-center sm:flex-row sm:gap-16 lg:gap-32 ">
          {authorsArray.map((item) => {
            return (
              <a
                key={item.nickname}
                className="text-xs text-white text-opacity-70 transition-colors hover:text-opacity-100 sm:text-base"
                href={item.src}
                target="_blank"
                rel="noreferrer"
              >
        ...
        <p className="p-4 text-white">2022</p>
      </div>
    </div>
  );
};
